...
export class BugOverlay {
  ...
  private async triggerQuickFix(bug: ExistingBug): Promise<void> {
    ...
    // Update bug status to in_progress (don't await, fire and forget)
    ...
    // Encode prompt as base64 for URL
    const promptB64 = btoa(unescape(encodeURIComponent(prompt)));
    const projectPath = bug.projectPath || '';

    // Open the bug details page on BugRadar dashboard where "Launch Claude" works
    const dashboardUrl = this.config.apiUrl.replace('/api/v1', '').replace('/api', '');
